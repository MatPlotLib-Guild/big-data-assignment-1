...
DB_CONFIG = dict(host="localhost", port=5433, dbname="foursquaredb", user="postgres", password="postgres")
DATA_DIR = Path("data")
...
def copy_tsv(cur, path: Path, table: str, columns: str) -> None:
    with open(path) as f:
        cur.copy_expert(
            f"COPY {table} ({columns}) FROM STDIN WITH (FORMAT CSV, DELIMITER E'\\t', HEADER TRUE)",
            f,
        )
